...
coarse_count <- aggregate(coverage_by_category_and_time$bact_count, by=list(gsub("^(.).*", "\\1", rownames(coverage_by_category_and_time))), sum)

get_coarse_count <- function(letter){
	rows <- grepl(letter, rownames(coverage_by_category_and_time))
	sum(coverage_by_category_and_time[rows, "bact_count"])
}

coarse_counts <- sapply(c("A", "B", "P", "S", "Z", "O"), get_coarse_count)
coarse_percentages <- 100 * coarse_counts / sum(coarse_counts)
...
phylum_category <- read.table(file="data/process/phylum_category_counts.tsv", header=T)
...
per_cult_b_otus <- 100 * sum(b_cult_time$all_cult_sobs) / sum(b_cult_time$all_total_sobs)
...
bacteria <- phylum_category[phylum_category$domain == "bacteria", ]
rownames(bacteria) <- bacteria$phylum
bacteria <- bacteria[,-c(1,2, ncol(bacteria))]
...
archaea <- phylum_category[phylum_category$domain == "archaea", ]
rownames(archaea) <- archaea$phylum
archaea <- archaea[,-c(1,2, ncol(archaea))]

dominant_archaea <- sort(table(apply(archaea, 2, which.max)), decreasing=T)
...
***Data analysis.***
Our analysis made use of ARB (OS X v.6.0) [@Pruesse2007], mothur (v.1.37.0) [@Schloss2009], and R (v.`r paste(version$major, version$minor, sep='.')`) [@language2015]. Within R we utilized the knitr (v.`r packageDescription("knitr")$Version`), wesanderson (v.`r packageDescription("wesanderson")$Version`), and openxlsx (v. `r packageDescription("openxlsx")$Version`) packages. A reproducible version of this manuscript including data extraction and processing is available at https://www.github.com/SchlossLab/Schloss_Census2_mBio_2016.
...
**Supplementary Table 3. Frequency that each archaeal phylum was sequenced before and after 2006.**

**Supplementary Table 4. Frequency that each bacterial phylum was found across each of the environmental categories.**

**Supplementary Table 5. Frequency that each archaeal phylum was found across each of the environmental categories.**

**Supplementary Table 6. Frequency that each bacterial sequence or OTU was retrieved by cultivation or by culture-independent methods.**

**Supplementary Table 7. Frequency that each archaeal sequence or OTU was retrieved by cultivation or by culture-independent methods.**
