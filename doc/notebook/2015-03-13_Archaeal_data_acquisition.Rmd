...
#... and we output a group file to use for splitting up the list file
write.table(file="data/mothur/archaea.category.groups", x=cbind(rownames(metadata), metadata$category), quote=FALSE, row.names=FALSE, col.names=FALSE, sep="\t")
```

One thing we'd like to look at are the rarefaction curves for each environmental category. To do this we'll run the mothur command like so..

```{r, engine="bash"}
mothur "#set.dir(input=data/mothur, output=data/mothur);
        make.shared(list=all_archaea.filter.unique.precluster.an.list, group=archaea.category.groups, label=unique-0.03-0.05-0.10-0.20);
        rarefaction.single(shared=all_archaea.filter.unique.precluster.an.shared)"
```
