...
```{r format_taxonomy}
tax_data <- read.table(file="archaea.taxonomy.nds", sep="\t")
...
write.table(cbind(seq_names, no.subs.str), "archaea.tax", row.names=F,
...
data <- read.table(file="archaea.summary", header=T, row.names=1)
...
mothur "#set.current(processors=8);
        screen.seqs(fasta=archaea.fasta, taxonomy=archaea.tax, start=2000, end=4058, maxambig=2, maxhomop=8, minlength=900);"
```
...
mothur "#align.seqs(fasta=single_cell.archaea.fasta, reference=archaea.good.fasta);
...
cat archaea.good.fasta single_cell.archaea.good.align > all_archaea.align
cat archaea.good.tax single_cell.archaea.good.pds.wang.taxonomy > all_archaea.taxonomy

mothur "#filter.seqs(fasta=all_archaea.align, vertical=T, trump=.);
        unique.seqs();
        pre.cluster(fasta=current, name=current, diffs=9);"

#need to make the taxonomy file match the fasta and names files...
cut -f 1 all_archaea.filter.unique.precluster.names > precluster.accnos
mothur "#get.seqs(taxonomy=all_archaea.taxonomy, accnos=precluster.accnos)"
```
...
mothur "#cluster.split(fasta=all_archaea.filter.unique.precluster.fasta, name=all_archaea.filter.unique.precluster.names, taxonomy=all_archaea.pick.taxonomy, taxlevel=2, classic=T, processors=8)"
```
...
metadata <- read.table(file="archaea_metadata.nds", sep="\t", stringsAsFactors=FALSE,
...
```{r}
bad_accnos_data <- read.table(file="archaea.bad.accnos", col.names=c("name.accnos", "reason"))
bad_accnos <- bad_accnos_data$name.accnos
metadata_good <- metadata[-c(bad_accnos),]
write.table(metadata_good, file="all_archaea.good.metadata", quote=F, sep="\t")
```
...
spread_sheet <- read.xlsx("data/raw/ArchaealSSUMetadata.xlsx", sheet=1, startRow=1, colNames=TRUE)
bad_accnos_data <- read.table(file="data/process/single_cell.archaea.bad.accnos", col.names=c("name.accnos", "reason"))
bad_accnos <- bad_accnos_data$name.accnos
spread_sheet_good <- spread_sheet[-c(bad_accnos),]

write.table(spread_sheet_good, file="data/process/all_archaea.good.metadata", quote=F, sep="\t", col.names=F, append=T)
```
