...
This is redundant with what we saw for the bacterial data, but for
completion...
...
taxonomy provided by the other systems if we'd like. A quick spot
checking suggests that the RDP taxonomy information is actually richer
than that of the EBML taxonomy information.
...
The next thing we need to do is to clean up the archaea.taxonomy.nds
file to make it into a proper, mothur compatible archaea.taxonomy file.
...
Notes... \* Will want to get rid of sequences with large number of Ns in
them and the ridiculously long homopolymrs - how large?
...
to `archaea_metadata.nds`. We need to tweak this file slightly in R to
...
Now we want to read in `archaea.bad.accnos` so that we can figure out
...
concatenate to the end of `archaea.good.metadata`.
