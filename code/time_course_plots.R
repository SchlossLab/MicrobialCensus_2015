...
pdf(file="results/figures/time_course_figure.pdf", width=3.75, height=6.0)
	...
	plot(time_data$b_nseqs~year, log='y', type="l", col="blue", lwd=2, xlab="", ylab="Number of new\nsequences per year", ylim=c(1,1e6), xlim=c(1980, 2015), axes=F)

	points(time_data$a_nseqs~year, type="l", col="red", lwd=2)

	points(time_data$b_nseqs[year %% 5==0]~year[year %% 5==0], pch=17, col="blue", cex=1.5)
	points(time_data$a_nseqs[year %% 5==0]~year[year %% 5==0], pch=19, col="red", cex=1.5)

	axis(1, labels=FALSE)
	axis(2, at=c(1,10,1e2,1e3,1e4,1e5,1e6), labels=parse(text=paste("10^", 0:6)), las=1)
	box()
	legend(x=2003, y=100, legend=c("Bacteria", "Archaea"), col=c("blue", "red"), pch=c(17,19), lty=1, lwd=2, pt.cex=1.5)
	...
	plot(time_data$b_sobs~year, log='y', type="l", col="blue", lwd=2, xlab="", ylab="Number of new\nOTUs per year", ylim=c(1,20000), xlim=c(1980, 2015), axes=F)

	points(time_data$a_sobs~year, type="l", col="red", lwd=2)

	points(time_data$b_sobs[year %% 5==0]~year[year %% 5==0], pch=17, col="blue", cex=1.5)
	points(time_data$a_sobs[year %% 5==0]~year[year %% 5==0], pch=19, col="red", cex=1.5)
	...
	#C
	plot(time_data$b_n50~year, type="l", lwd=2, xlab="", ylab="Number of submissions accounting\nfor 50% of sequences deposited", xlim=c(1980, 2015), col="blue", axes=F)

	points(time_data$a_n50~year, type="l", lwd=2, col="red")

	points(time_data$b_n50[year %% 5==0]~year[year %% 5==0], pch=17, col="blue", cex=1.5)
	points(time_data$a_n50[year %% 5==0]~year[year %% 5==0], pch=19, col="red", cex=1.5)
