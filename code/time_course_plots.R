...
summary_table <- data.frame(matrix(rep(0, 32*8), ncol=8))
rownames(summary_table) <- start_year:(year_of_analysis-1)
colnames(summary_table) <- c('b_nseqs', 'b_sobs', 'b_n50', 'b_phyla',
							'a_nseqs', 'a_sobs', 'a_n50', 'a_phyla')
...
pdf(file="results/figures/sequences_deposited.pdf")
	...
	summary_table[names(bact_deposits_by_year),'b_nseqs'] <- bact_deposits_by_year
	...
	summary_table[names(arch_deposits_by_year),'a_nseqs'] <- arch_deposits_by_year
...
pdf(file="results/figures/otus_deposited.pdf")
	...
	summary_table[names(new_bact_otus_by_year),'b_sobs'] <- new_bact_otus_by_year
	...
	summary_table[names(new_arch_otus_by_year),'a_sobs'] <- new_arch_otus_by_year
...
pdf(file="results/figures/phyla_deposited.pdf")
	...
	summary_table[names(date_bact_phylum_first_appear),'b_phyla'] <- date_bact_phylum_first_appear
	...
	summary_table[names(date_arch_phylum_first_appear),'a_phyla'] <- date_arch_phylum_first_appear
...
pdf("results/figures/fifty_authors_deposited.pdf")
	...
	summary_table[names(bact_fifty),'b_n50'] <- bact_fifty
	...
	summary_table[names(arch_fifty),'a_n50'] <- arch_fifty
