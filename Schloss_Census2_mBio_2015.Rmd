...
#	aquifer1 - scrap
#	bact[bact_phylum=="Candidate_division_OP3" & !is.na(bact$isolate),] - AJ224540.MxeCultu
...
cult_otus <- unique(bact$otu[b_is_cult])			# otus with a cultured representative
b_in_cult_otu <- bact$otu %in% cult_otus


#total number of OTUs by phylum
n_all_otus_phylum <- aggregate(bact$otu, by=list(bact_phylum), function(x){length(unique(x))})

#total number of OTUs with a cultured rep by phylum
n_cult_otus_phylum <- aggregate(bact$otu[b_in_cult_otu], by=list(bact_phylum[b_in_cult_otu]), length)

#need to merge these two tables...
...
* % of OTUs with a cultured representative by phylum
	- basically done.



* do these new cultures represent new OTUs?
