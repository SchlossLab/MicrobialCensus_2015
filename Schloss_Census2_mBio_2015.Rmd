...
Given the shift away from Sanger sequencing to platforms that offer higher throughput but with shorter reads, there is concern that our ability to harvest full-length sequences from communities will remain stalled. Several culture-idnependent methods have been developed that offer the ability to obtain full-length sequences of the 16S rRNA gene and even the complete genome. One option is the use of single cell genomics where uncultured bacteria are isolated by dilution and then their genomes are amplified and sequenced. To test the ability of single cell technologies to expand our knowledge of microbial diversity beyond that of the 16S rRNA gene and pure cultures, we compared the overlap of OTUs found by the three methods (Figure results/figures/venn_otu_by_method.pdf). Utilizing the 16S rRNA gene sequences extracted from the single-cell genomes available on the Integrated Microbial Genomes (IMG) system [@REF], we identified `r overlap_counts['n_sc', 'bact_counts']` bacterial and `r overlap_counts['n_sc', 'arch_counts']` archaeal sequences that met our criteria, which were assigned to `r overlap_counts["sc_all", "bact_counts"]` and `r overlap_counts["sc_all", "arch_counts"]` bacterial and archaeal OTUs, respectively. Interestingly, only `r b_per_unique_sc` and `r a_per_unique_sc`% of the bacterial and archaeal OTUs, respectively, that the single-cell 16S rRNA gene sequences belonged to had not been observed by cultivation or PCR-based efforts. For both domains, the fractions of the single-cell 16S rRNA gene sequences that were recovered by cultivation and PCR were similar. Furthermore, among the bacteria `r b_per_shared`% of the single-cell OTUs were previously observed by both cultivation and PCR-based methods. Although the majority of single-cell genome projects have originated from a single study [@REF], `r b_expand_genome` and `r a_expand_genome` of the bacterial and archaeal OTUs, respectively, were from previously uncultured organisms. Another approach is to assemble sequence reads from metagenomic sequencing projects that correspond to the 16S rRNA gene. This has been difficult because of the risk of generating assembly artifacts as a result of the high level of similarity among conserved domains within 16S rRNA genes. The development of the EMIRGE algorithm has facilitated the accurate assembly of 16S rRNA gene fragments [@REF]. We obtained XXX sequences from 3 studies that assembled 16S rRNA genes from metagenomes, which belonged to XX OTUs. Comparing the OTUs found from the metagenomes to the other methods we found no overlap with the OTUs generated by single cell genome sequencing 
