...
One explanation for the large number of OTUs that have only been observed once is that with the the broad adoption of highly parallelized sequencing platforms that generate short sequence reads, the rate of full-length sequence generation has declined. In fact, since 2009 the number of new bacterial sequences generated has slowed two an average of `r floor(mean(time_data[as.character(2009:2014),'b_nseqs']))` sequences per year (Figure 2A). Although this is still an impressive number of sequences, since `r start<-2007;start` the number of new bacterial OTUs has plateaued at an average of `r mean(time_data[as.character(start:2014),'b_sobs'])` new OTUs per year (Figure 2B). Given the expense of generating full-length sequences using the Sanger sequencing technology and the transition to other platforms, we expected that the large number of sequences were being deposited by a handful of large projects. Indeed, when we counted the number of submissions responsible for depositing 50% of the sequences, we found that with the exception of 2006 and 2013, eight or fewer studies were responsible for depositing the majority of the full-length sequences each year since 2005 (Figure 2C). Between 2009 and 2012, `r sum(time_data[as.character(2009:2012), 'b_nseqs'])` total sequences were submitted and `r sum(time_data[as.character(2009:2012), 'b_n50'])` submissions from `r length(unique(dominant_titles))` studies were responsible for depositing `r dominant_studies` (`r 100*dominant_studies/sum(time_data[as.character(2009:2012), 'b_nseqs'])`% of all sequences). These studies generated sequences from the human gastrointestinal tract [@Li_2012], human skin [@Kong_2012; @Grice_2009], murine skin [@Grice_2010], and hypersaline microbial mats [@Kirk_Harris_2012]. In contrast to recent years, between 1995 and 2006, an average of `r mean(time_data[as.character(1995:2006), 'b_n50'])` studies were responsible for submitting more than half of the sequences each year. Although these deep surveys represent significant contributions to our knowledge of bacterial biogeography, their small number and lack of environmental diversity is indicative of the broader problems in advancing the bacterial census.


***The dynamics of the archaeal census.***
The depth of sequencing being done to advance the archaeal census is `r floor(sum(time_data$b_nseqs)/sum(time_data$a_nseqs))`-times less than that of the bacterial census (Table 1). The annual number of sequences submitted has largely paralleled that of the bacterial census with a plateau starting in 2009 and an average of `r mean(time_data[as.character(2009:2014),'a_nseqs'])` sequences each year since then. The number of new archaeal OTUs represented by these sequences began to slow in 2005 with an average of `r mean(time_data[as.character(2005:2014),'a_sobs'])` new OTUs per year. With the exception of 2012 and 2014, the number of submissions responsible for more than 50% of the archaeal sequences submitted per year has varied between `r min(time_data[c(1996:2011,2013),'a_n50'])` and `r max(time_data[c(1996:2011,2013),'a_n50'])` submissions per year. The clear bias towards sequencing bacterial 16S rRNA genes has limited the ability to more fully characterize the biodiversity of the archaea.

```{r environment}
	strip <- function(x){	as.numeric(gsub(',', '', x))	}

	table1_raw <- read.table(file="data/process/table1_data.tsv", row.names=NULL, stringsAsFactors=FALSE)
	table1_raw <- table1_raw[-nrow(table1_raw),]
	rownames(table1_raw) <- table1_raw$row.names
	rownames(table1_raw)[nrow(table1_raw)] <- 'no_source_data'
	table1_raw <- table1_raw[,-c(1,2)]
	table1_raw[c("AE", "OT", "no_source_data"),"fine"] <- c("Aerosol", "Other", "no_source_data")

	for(col in 2:ncol(table1_raw)){
		table1_raw[,col] <- strip(table1_raw[,col])
	}

	table1_good <- table1_raw[-nrow(table1_raw),]

	b_class_percent <- 100*sum(table1_good$bact_count)/sum(table1_raw$bact_count)
	a_class_percent <- 100*sum(table1_good$arch_count)/sum(table1_raw$arch_count)

	coarse_map <- c("A"=="aerosol", "AQ" = "aquatic", "B" = "built environment", "O" = "other", "P" = "plant", "S" = "soil", "Z" = "zoonotic")

	coarse <- gsub("^(.)[A-Z]$", "\\1", rownames(table1_good))
	coarse <- gsub("^(..)[A-Z]+$", "\\1", coarse)

	b_agg_count <- aggregate(table1_good$bact_count, by=list(coarse_map[coarse]), sum)
	a_agg_count <- aggregate(table1_good$arch_count, by=list(coarse_map[coarse]), sum)


```

***Focusing the census by environment***
We were able to assign `r b_class_percent` and `r a_class_percent`% of the sequences to one of seven broad environmental categories based on the metadata that accompanied the SILVA database. Across these broad categories there was wide variation in the number of sequences that have been sampled. Among the bacterial sequences the three best represented groups were from `r b_agg_count[1]`, `r b_agg_count[2]`, and `r b_agg_count[3]` sources and among the bacterial sequences the three best represented groups were from `r a_agg_count[1]`, `r a_agg_count[2]`, and `r a_agg_count[3]` sources. For both domains, soil samples were the fourth most represented category. The orders of these categories was surprising considering soil and aquatic environments harbor the most microbial biomass and biodiversity [@Whitman1998]. In spite of wide variation in sequencing depth and coverage (Table 1), the interquartile range for the bacterial OTU-based coverage only varied between `r quantile(table1_good$bact_schloss_coverage, prob=c(0.25))` to `r quantile(table1_good$bact_schloss_coverage, prob=c(0.75))` categories (median coverage=`r median(table1_good$bact_schloss_coverage, na.rm=T)`%). The interquartile range in the OTU-based coverage by environment for the archaeal data was `r quantile(table1_good$arch_schloss_coverage, prob=c(0.25), na.rm=T)` to `r quantile(table1_good$arch_schloss_coverage, prob=c(0.75), na.rm=T)` (median coverage=`r median(table1_good$arch_schloss_coverage, na.rm=T)`%). The archaeal coverage was higher than that of the bacterial OTU coverage for all categories except the food-associated, plant surface, and other invertebrate categories. Given the relatively low number of archaeal sequences that have been deposited for many of these categories, it is possible that the coverage for many categories may not be reliable (e.g. aerosol and plant surfaces). By decomposing the census by environmental categories, it is clear that even among the best sampled environments, our ability to claim anything but a basic characterization of microbial biodiversity is limited.
...
```{r culture}
	...
	b_ncult_2005 <- sum((!is.na(bact$strain) | !is.na(bact$isolate)) & bact_year_deposited <= 2005, na.rm=T)
	a_ncult_2005 <- sum((!is.na(arch$strain) | !is.na(arch$isolate)) & arch_year_deposited <= 2005, na.rm=T)
...
#	bact[bact_phylum=="CKC4" & !is.na(bact$isolate),] -> 2 - 1 is a genome, 1 is an isolate from same research group
...
#	bact[bact_phylum=="TM6" & (!is.na(bact$isolate) | !is.na(bact$strain)) & !grepl("\\.Unc", rownames(bact)),] - legit
	...
	b_n1_cult_otus <- sum(table(bact$otu[b_in_cult_otu])==1)/length(table(bact$otu[b_in_cult_otu]))
	a_n1_cult_otus <- sum(table(arch$otu[a_in_cult_otu])==1)/length(table(arch$otu[a_in_cult_otu]))
```

***The cultured census***
In our previous analysis there was great concern that although culture-independent methods were significantly enhancing our knowledge of microbial life, there were numerous bacterial phyla with no or only a few cultured representatives.

* fraction of sequences from cultured / uncultured bugs (2005 vs 2014) by domain
	- 18.9% of bacterial sequences and 7.2% of archaeal sequences are from isolates (b_fract_cult & a_fract_cult)
	- culture rates lagging total submissions by 2.7-fold (bacteria / archaea; b_slow_cult / a_slow_cult)
...
* do these new cultures represent new OTUs by phylum and in aggregate
	- b_n1_cult_otus
	- a_n1_cult_otus
...
***The census is taxonomically skewed***
...
```{r taxon_census}
	...
	b_new_phyla <- colnames(b_phylum_year)[apply(b_phylum_2006, 2, sum) == 0] #what are the new phyla?
	b_new_phyla_names <- paste(paste(b_new_phyla[1:(length(b_new_phyla)-1)], collapse=", "),
							 b_new_phyla[length(b_new_phyla)], sep=", and ")
	...
	a_new_phyla <- colnames(a_phylum_year)[apply(a_phylum_2006, 2, sum) == 0] #what are the new phyla?
	a_new_phyla_names <- paste(a_new_phyla, collapse=" and ")
...
* For which phyla has there been substantial increases in sampled diversity since 2004?
	- new phyla: b_new_phyla_names, a_new_phyla_names
	- Figure phylum_effort.pdf

* generate a heatmap of category by phylum relative abundance
	- Figure phylum_category_heatmap
