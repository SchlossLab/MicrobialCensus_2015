...
```{r taxon_census}
	...
	b_phylum_n <- apply(b_phylum_year, 2, sum)
	...
	a_phylum_n <- apply(a_phylum_year, 2, sum)
...
***The ability to sample microbial life is taxonomically skewed*** <-- Sucky title
The Firmicutes, Proteobacteria, Actinobacteria, and Bacteroidetes represent `r 100*sum(b_phylum_n[c("Firmicutes", "Proteobacteria", "Actinobacteria", "Bacteroidetes")])/sum(b_phylum_n)`% of the bacterial sequences and the Euryarchaeota and Thaumarchaeota `r 100*sum(a_phylum_n[c("Euryarchaeota",  "Thaumarchaeota")])/sum(a_phylum_n)`% of the archaeal sequences, yet we sought to understand how the representation individual phyla has changed relative to the state of the census in 2006. We used 2006 as a reference point for calibrating the dynamics of the bacterial and archaeal censuses since that was the year that the first highly parallelized 16S rRNA gene sequence dataset was published and ushered in a radical change in how microbial communities are studied [@Sogin2006]. Since then, the number of phyla within each domain has continued to increase. In 2006 there were `r b_n_phylum_2006` bacterial and `r a_n_phylum_2006` phyla. Since then there have been `r b_new_phyla` new  bacterial (`r b_new_phyla_names`) and `r a_new_phyla` new archaeal candidate phyla (`r a_new_phyla_names`). Relative to the overall sequencing trends before and after 2006, several phyla stand out for being over and underrepresented in sequence submissions (Figure phylum_effort.pdf). Among the bacterial phyla with at least 1,000 sequences, Kazan−3B−09 (`r something`) and Atribacteria (`r something`) were sequenced 4-fold more often while Deinococcus−Thermus (`r something`) and Tenericutes (`r something`) were sequenced 4-fold less often than would have been expected since 2006. Among the archaeal phyla with at least 1,000 sequences, the Thaumarchaeota were sequenced `r something`-fold more often and the Crenarchaeota were sequenced `r something`-fold less often than expected. Overall, these results suggest <something...>
